        ...
        for j, elements in enumerate(wordElem):
            ...
            if not elements:
                ...
                if len(elements) == 1:
                    ...
                    tmp_prods = [prod for k, prod in enumerate(deepcopy(prods)) if prod_lens[k] != 0]
...
def gen_candiate(word_elements, words, lemmas, keywordpos, pat_poslen):    
    ...
    for elem_i, elements in enumerate(word_elements):
        ...
        # if pos and pos & set(['VERB']) and (words[elem_i] in aklWords or lemmas[elem_i] in aklWords):
            pos = pos.pop()
            # for (pat_len, pos_index), pats in Colpatterns.ColPat_poslen[pos].iteritems():
            for (pat_len, pos_index), pats in pat_poslen[pos].iteritems():
...
def get_prev_aftr_word(words, chunkposs, index):
    ...
    time = 0
    for i in reversed(range(0, index[0])):        
        if i == 0:
            prev_index = 0
        elif chunkposs[i][0] == 'H':
            time += 1
        if time == 2:
            if re.search('[^a-zA-Z0-9_]+', words[i+1]): prev_index = i+2
            else: prev_index = i+1
            break
    # if prev_index > -1: prev = words[prev_index]+' '
    if prev_index > -1: prev = ' '.join(words[prev_index:index[0]])+' '
    else: prev = ''
    ...
    if aftr_index != -1: aftr = ' '+' '.join(words[index[-1]+1:aftr_index+1])
    else: aftr = ''
    return prev, aftr
            ...
            if chose == 'combine':
                ...
                for pos, elem_i in gra_pat_dict:
                    for gra_cand, gra_index in gra_pat_dict[(pos, elem_i)]:
                        ...
                        gra_dict = dict(izip(gra_index, gra_cand))                        
                        if col_pat_dict[(pos, elem_i)]:
                            for col_cand, col_index in col_pat_dict[(pos, elem_i)]:
                                ...
                                # if True:
                                    # print col_index, gra_index, not set(col_index) <= set(gra_index)
                                    ...
                                    patdata = '{}:{}\t{}\t{}\t{}'.format(lemmas[elem_i].lower(), pos_map[pos], pat, iocs, example)
                                    table[patdata] += 1
                                    print patdata
            else:    
                if chose == 'grammar':
                ...
                for pos, elem_i, cand, index in gen_candiate(wordElement, words, lemmas, keywordpos, pat_poslen):
                    ...
                    table[patdata] += 1
                    # print patdata
    for patdata, cnt in table.iteritems():
