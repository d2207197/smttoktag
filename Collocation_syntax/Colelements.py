...
with open('EnglishPhrasalVerb.txt') as fpr:
    VerbPhrases = defaultdict(lambda:defaultdict(list))
    for line in fpr:
        if line.rstrip():
            VerbPhrases[line.split('_', 1)[0]][line.count('_')+1].append(line.rstrip().split('_', 1)[1])
# print VerbPhrases
with open('writeaway_word.txt') as fpr:
...
head_map = {
    'NN': ['sth'], 'NNS': ['sth'], 'NNP':['sth'], 'NNPS':['sth'],
    # 'VB':['do', 'inf'], 'VBZ':['do'], 'VBD':['do'], 'VBG':['do', 'doing'], 'VBN':['do', 'done'], 'VBP':['do'],
    'VB':['do'], 'VBZ':['do'], 'VBD':['do'], 'VBG':['do'], 'VBN':['do'], 'VBP':['do'],
    'JJ': ['adjp'], 'JJR': ['adjp'], 'JJS': ['adjp'], 'RB':['advp', '_'], 'RBR':['advp', '_'], 'RBS':['advp', '_'], 
    'CD': ['amount'],
    'TO':['to', 'prep'],       # to/prep
    'DT': ['THIS', 'sth'],      # this
    'MD': ['_'],
    'SYM': ['_'], 'LS': ['_'], 'HYPH': ['_'], 'POS': ['_'], 'UH': ['_'],
    'FW': ['sth'],
    'IN':['prep'], 'WDT': ['wh'], 'WP': [ 'wh'], 'WRB': [ 'wh'],    
    'CC': ['and'], 'EX': ['there'], 
    'PRP':['sth'], 'PRP$': ["'one's", '_'], 
}

nonhead_map = {
    'NN': ['n', '_'], 'NNP': ['n', '_'], 'NNS': ['n', '_'], 'NNPS': ['n', '_'],
    'VB': ['v', '_'], 'VBP': ['v', '_'], 'VBG': ['v', '_'],
    'VBZ': ['v', '_'], 'VBN': ['v', '_'], 'VBD': ['v', '_'],
    'JJ': ['adj', '_'], 'JJR': ['adj', '_'], 'JJS': ['adj', '_'],
    'RB':['adv', '_'], 'RBR':['adv', '_'], 'RBS':['adv', '_'], 
    'DT': ['det.', '_'], 
    'IN': ['?'], 'CD':['amount', '_'],
    'PRP': ['sth'], 'PRP$': ["'one's", '_'],     
    'MD': ['_'], 'WP': ['wh'], 'WRB': ['wh'],
...
def word2Element(words, lemmas, poss, chunkposs):
    ...
    pat = []
    verb_tag = 0
    for i, (word, lemma, pos, chunkpos) in enumerate(izip(words, lemmas, poss, chunkposs)):
        element = []
        ...
        elif re.match('V(B|BD|BN|BG|BP)', pos) and i+1 < sentlen and poss[i+1] == 'TO' and chunkposs[i+1][0] == 'I':
            element.extend(head_map[pos])
        elif i+2 < sentlen and chunkpos == 'H-SBAR' and lemma == 'in' and lemmas[i+1] == 'order' and lemmas[i+2] == 'to':
            element.append('_')
        elif i > 0 and i+1 < sentlen and chunkposs[i-1] == 'H-SBAR' and lemmas[i-1] == 'in' and lemma == 'order' and lemmas[i+1] == 'to':
            element.append('_')
        ...
        elif i+1 < sentlen and words[i+1] == "'s" and re.match('N(N|NS|NP|NPS)', pos):
        ...
        if lemma in VerbPhrases:
            for length in sorted(VerbPhrases[lemma].iterkeys(), reverse=True):
                if ' '.join(lemmas[i+1:i+length]) in VerbPhrases[lemma][length]:
                    verb_tag = length-1
                    break
        # print word, element
        pat.append(element)
...
def testing():
    ...
    for line in input_lines:
        ...
        words, lemmas = map(methodcaller('split'), (words, lemmas))
        if set(words+lemmas) & aklWords:
            poss, chunkposs = map(methodcaller('split'), (poss, chunkposs))
            ...
            for word, element in izip(words, wordElement):
